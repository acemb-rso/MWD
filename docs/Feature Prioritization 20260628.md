# Feature Prioritization: MWD Next Steps

> **Status review 2026-06-28.** Most of the original (2026-04-14) prioritized
> work has since shipped. This revision marks each item against the current
> code. The genuinely remaining gaps are now small and listed up front under
> [Remaining Work](#remaining-work); the tier sections below are kept as a
> record of what was delivered.

## Remaining Work

The only features still incomplete are the explicit runtime stubs and one broad
automation area:

- **Dodge** personal combat action — stub. [personal-action-catalog.js:232](../src/modules/combat/personal-action-catalog.js#L232) ("Dodge mechanics are not implemented yet.")
- **React** generic reaction — stub; routes users to specific reactions instead. [personal-action-catalog.js:234](../src/modules/combat/personal-action-catalog.js#L234)
- **Manual ejection** machine action — stub. [machine-action-catalog.js:346](../src/modules/mwd/machine-action-catalog.js#L346) ("Manual ejection state changes are not automated yet.")
- **Weapon trait automation** — broad, ongoing. Payload/profile modifiers are live (damage type, AP, attack-rating bands, templated payload capabilities) and personal fire modes are fully implemented, but standard weapon *traits* still need concrete, rule-backed, tested behavior before registry entries are enabled. See Tier&nbsp;1 #3 below.

Separately tracked (not part of this doc's scope): HUD token shortcuts remain
broken after the legacy class cleanup.

## Context

The system architecture is mature enough that the highest-value work is now targeted mechanics and sheet polish. The `system.mwd` block is present in `template.json`, and vehicle/BattleMech actors have the Handling/System/Chassis/Condition attribute set.

The machine hit-location and critical-hit handoff is implemented through the current resolver, chat, and HarmEngine pipeline. Older notes that described this as a sidecar combat subsystem are superseded by the queued `machineAttackDamage` mutation contract.

## Current Core Gameplay State

Vehicle and BattleMech attacks now resolve successful machine hits into queued chat-card consequences:

- the resolver attaches `type: "machineAttackDamage"` mutations to non-miss machine targets
- hit location keeps both `impactLabel` and grouped `rulesLocation`
- critical state is represented by `critical.mode`
- automatic and Chaos-selected crits prepare revision-pinned crit records during preview
- Reliability spend is previewed as a pending choice
- `HarmEngine.applyMachineAttackDamage` is the only writer for armor, structure, stress, shock/pressure, Reliability, degradation, crit records, and status
- repeat apply is idempotent

The current implementation contract is documented in:

- [critical-hit.md](critical-hit.md)
- [Critical-hit-plan.md](Critical-hit-plan.md)
- [combat-flow-design.md](combat-flow-design.md)

## Recommended Feature Order

## Tier 1 - Core Gameplay Blockers

These are the next features most likely to improve vehicle and BattleMech play at the table.

### 1. Vehicle And BattleMech Sheet Surfacing - Implemented

`VehicleSheetV2` and `BattlemechSheetV2` are now fully layout-driven. The
BattleMech layout surfaces Quick Actions, Combat Awareness, Fire Mode, Weapon
Groups, Chassis, Attributes, Signature, Movement, Structure, Criticals,
Degradation, Heat, Pilot, Hardpoints, Equipment, Modules, and Gear; the Vehicle
layout mirrors the relevant subset. Quick-action buttons remain intent emitters
routed through the existing roll payloads.

Relevant files:

- `templates/v2/layouts/battlemech.layout.json`
- `templates/v2/layouts/vehicle.layout.json`
- `src/modules/sheets/vehicle-sheet-v2.js`
- `src/modules/sheets/battlemech-sheet-v2.js`

### 2. Machine Crit And Degradation UX Polish - Implemented

The machine sheets now carry dedicated **Criticals** (`mwd.v2.ui.vehicle.active-crits`)
and **Degradation** (`mwd.v2.ui.vehicle.degradation-grid`) panels. Remedy actions
with action costs are exposed through the machine action catalog and crit-remedy
service. Some reminder-only crit effects still lack provider hooks and surface as
status callouts rather than automation.

Relevant files:

- `src/modules/mwd/machine-crit-remedies.js`
- `src/modules/mwd/machine-crit-consequences.js`
- `src/modules/mwd/machine-degradation.js`
- `templates/v2/layouts/battlemech.layout.json`

### 3. Weapon Trait Automation - In Progress

Weapon and payload traits remain a broad automation area. Payload/profile modifiers are active for damage type, AP, attack-rating band changes, and templated payload capabilities, but standard trait behavior still needs concrete rule-backed implementation.

Recommended work:

- Prioritize traits that affect the existing resolver contract: dice parts, DN parts, CQ parts, damage preview, and queued mutation metadata.
- Avoid enabling registry entries until each trait has an observable behavior and tests.

## Tier 2 - High Value, Standalone

### 4. Item Sheet Layouts - Implemented

All canonical item types now have full JSON layout coverage, including the types
previously called out as missing: `skill`, `quality`, `gear`, `contact`,
`assetModule`, `lifeModule`, and `mechWeapon`.

Relevant files:

- `templates/v2/layouts/skill.layout.json`
- `templates/v2/layouts/quality.layout.json`
- `templates/v2/layouts/gear.layout.json`
- `templates/v2/layouts/contact.layout.json`
- `templates/v2/layouts/asset-module.layout.json`
- `templates/v2/layouts/life-module.layout.json`
- `templates/v2/layouts/mech-weapon.layout.json`

### 5. Area Effects - Exposure UI And Hazard Tools - Mostly Implemented

The four-tier exposure model (Full / Major / Minor / None) with multipliers is
live, and a continuous area-status runtime (sources, ownership, hidden Regions,
reconciliation) plus hazard chat affordances were added after the original
prioritization. Remaining polish is primarily GM-facing authoring ergonomics and
any additional token-overlay surfacing.

Relevant files:

- `src/modules/area-effects/area-effect-engine.js`
- `src/modules/area-effects/hazard-chat.js`
- `src/modules/area-status/area-status-controller.js`
- `src/modules/area-status/area-status-sources.js`
- `src/modules/area-status/area-status-regions.js`
- `src/modules/area-status/area-status-ownership.js`

### 6. Character Data Completions - Implemented

XP advancement is a complete service: experience tiers plus validated, priced
purchases for attributes, skills, edge pools, traits, specializations, and
knowledge skills. Sheets emit purchase intents into this module.

Relevant files:

- `src/modules/advancement/character-advancement.js`
- `src/modules/sheets/character-sheet-v2.js`

## Tier 3 - Polish And Completeness

### 7. First Aid Combat Action - Implemented

The personal combat action catalog routes First Aid through `combatIntent`.
The central executor dispatches to the existing First Aid resolver path, so the
sheet stays declarative while the mature First Aid flow remains intact.

Implemented behavior:

- First Aid costs 2 SA as a Complex Action.
- The launcher prompts for target, recovery track, condition DN, and medical gear.
- The roll uses MedTech with DN 1/2/3 for Excellent/Normal/Poor conditions.
- Self-treatment applies -2 dice.
- Medical gear adds its rating to the dice pool.
- Chat card recovery is applied after the roll:
  - Fatigue: 1 recovered point per net hit.
  - Physical: 1 recovered point per 2 net hits after the first.
- Post-roll Edge remains available until recovery is applied, then is blocked for that card.

Relevant files:

- `src/modules/mwd/first-aid.js`
- `src/modules/modifiers/providers/first-aid.js`
- `src/modules/combat/personal-action-catalog.js`
- `src/modules/roll/intent/resolve-skill.js`
- `src/modules/sheets/base-actor-sheet-v2.js`
- `src/modules/sheets/character-sheet-v2.js`
- `src/modules/roll/renderers/render-skill.js`
- `src/modules/chat/chat-actions.js`

### 8. Weapon Mounting Validation - Implemented

Mount-point budget and melee-limit enforcement run during loadout computation:
`usedMountPoints > mountPointTotal` produces a localized error, and the loadout
reports used/remaining mount points back to the sheet.

Relevant files:

- `src/modules/mwd/battlemech-loadout.js`
- `src/modules/mwd/machine-hardpoints.js`
- `docs/weapon-mounting-system.md`

### 9. Legacy Cleanup

Older `.hbs` templates and Anarchy terminology can be trimmed once their modern replacements are fully wired. This reduces confusion but should follow behavior-complete replacements, not precede them.

## Verification Targets

For machine hit and critical work:

- A hit against a vehicle or BattleMech queues `machineAttackDamage`.
- A miss queues no machine damage mutation.
- Hit location includes both `impactLabel` and `rulesLocation`.
- `critical.mode` drives Chaos visibility and prepared crit requirements.
- Prepared crit records are pinned to `previewRevision`.
- Stale prepared crit records are refused.
- Chaos and Reliability toggles rebuild derived preview state.
- Reliability preview does not mutate actor state.
- Applying the same queued mutation twice does not duplicate damage, stress, degradation, Reliability spend, or crits.

For sheet and UX work:

- Vehicle and BattleMech actors render their machine monitors and state without console errors.
- Active crits and remedies are visible after apply.
- Character and personal combat flows remain unchanged.
