# Actor sheet orchestration notes

These notes capture where the actor sheet lifecycle can stumble and what to check when sheets refuse to open or render cleanly.

## Lifecycle refresher
 - The system startup wiring sets `CONFIG.Actor.documentClass` and registers sheet classes during the `init` hook. The live document class is `MWDActor` and the AppV2 sheet base options are provided by `BaseActorSheetV2` (`src/modules/sheets/base-actor-sheet-v2.js`).  
 - Actor documents are instantiated as `MWDActor` instances which handle per-type behavior internally; there is no per-type actor-class reroute in the current code. Verify the actor `type` value matches a registered sheet type.  
 - Sheets are registered by `registerActorSheetsV2()` (see `src/modules/sheets/register-actor-sheets-v2.js`); the active sheet for a given actor type is whichever class was registered as default or manually chosen per actor.

## Fragile spots to watch
 - **Actor type drift**: `BaseActorSheetV2.template` now logs when an actor type is missing and falls back to the character template instead of crashing. Verify actor JSON includes a supported `type`, and confirm registered sheets cover that type.
 - **Template preload hygiene**: Template paths gathered during render are trimmed and non-string or empty values are filtered out, with warnings and diagnostics that show exactly what was removed. If a sheet still fails to render, check the `templates-preload` log emitted by `preloadTemplatesV2()` (see `src/modules/sheets/preload-templates.js`) for the final list of templates being loaded (even when the list is empty).
- **Loader sanitization**: A system-level guard now strips any null/empty template paths before they reach `loadTemplates`, logging the offender plus a stack trace so unexpected call sites can be fixed before they trigger the `startsWith` crash.
- **System readiness**: Render logging records when `game.system.anarchy` is unavailable. If sheets break on world start, confirm `AnarchySystem.start()` ran (look for the init logs) before actors are touched.
- **Template expectations**: The sheet diagnostics include the resolved template path and whether system data (`actor.system`) is present. Mismatched template paths or stripped `system` data will show up there.

## Quick checklist before chasing render bugs
1. **Actor type & template** – Confirm the actor `type` matches a template file under `templates/actor/` and that a sheet was registered for that type. Fallback logging now highlights when the type is missing.
2. **Document class wiring** – Ensure `CONFIG.Actor.documentClass` points at `MWDActor` and that the actor `type` value matches a registered sheet class (see `src/modules/sheets/register-actor-sheets-v2.js`).
3. **Sheet registration** – Check `registerActorSheetsV2()` and the `anarchy-system` initialization to verify the desired sheet class is registered (and defaulted) for the actor type.
4. **System readiness** – Look for the `${LOG_HEAD}AnarchySystem.onInit`/`onReady` console logs. If sheets render before those fire, gating logic may need to delay rendering.
5. **Console diagnostics** – Use the new `ActorSheet` debug logs to capture `actorId`, `actorType`, template path, ownership IDs, and render timing. Rendering failures will now surface a notification and a structured error payload in the console.

## What “Cannot read properties of null (reading 'startsWith')” really means
- Foundry calls `startsWith` on every template path before loading it; that error means **a `null` or empty template path made it into the preload list**.
- Typical culprits:
  - An actor type that resolves to `templates/actor/<type>.hbs` where `<type>` is missing or blank, producing `templates/actor/.hbs`.
  - Custom sheet code that returns non-string entries from `_getTemplatePaths`.
- How to confirm quickly:
  - Check the `templates-preload` diagnostic log emitted by `preloadTemplatesV2()` (see `src/modules/sheets/preload-templates.js`) for any empty strings or unexpected entries.
  - Verify the resolved template path in the `render-start` log (it prints `template: ...`). If it shows the character fallback despite expecting another type, the actor data is likely missing `type`.
- Fast mitigations:
  - Fix the actor’s `type` value or rename the corresponding template so the resolved path is non-empty.
  - Ensure overrides of `_getTemplatePaths` only return trimmed strings; any other values are filtered and logged so they can be corrected before they hit Foundry’s loader.
